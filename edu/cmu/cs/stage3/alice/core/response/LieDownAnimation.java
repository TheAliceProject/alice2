...
public class LieDownAnimation extends BetterStandUpAnimation {
	...
	public class RuntimeLieDownAnimation extends RuntimeBetterStandUpAnimation {
		...
		public void prologue( double t ) {	
			...
			if( target == null ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("needs_something_or_someone_to_lie_down_on_", m_subject.name.getStringValue()), null, LieDownAnimation.this.target ); 
            }
            if( m_subject == target ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("can_t_lie_down_on_", m_subject.name.getStringValue(), target.name.getStringValue()), getCurrentStack(), LieDownAnimation.this.target );              
            }
            
            if (m_subject.isAncestorOf(target)) {
            	throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("can_t_lie_down_on_a_part_of_itself", m_subject.name.getStringValue()), getCurrentStack(), LieDownAnimation.this.target );             
            }
