...
public class SimpleSound{
	...
	public void writeToFile(String outFileName) throws SoundException
	{
		...
		{
			printError(Messages.getString("Unable_to_reset_the_Audio_stream___Please_try_again_"), e); 
		}//catch
		...
		if(!file.exists())
		{
			...
			{
				printError(Messages.getString("That_file_does_not_already_exist__and_", outFileName), e);  
			}//catch
		}//if
		...
		try
		{
			...
			{ 
				printError(Messages.getString("Problems_writing_to_file___Please_try_again_")); 
			}
		...
		{
			printError(Messages.getString("The_file_you_specified_did_not_already_exist_"), e); 
		}
		catch(Exception e)
		{ 
	...
	public void loadFromFile(String inFileName) throws SoundException
	{
		...
		if(inFileName == null)
		{
			printError(Messages.getString("You_must_pass_in_a_valid_file_name___Please_try_again_"));   
		}
		...
		if(!file.exists())
		{
			printError(Messages.getString("doesn_t_exist", inFileName));  
		}
		...
		try {
			...
			printError(Messages.getString("Unable_to_read_from_file_", inFileName) ,e); 
			return;
		}//catch
		...
		{
			printError(Messages.getString("The_sound_in_file__is_too_long_Try_using_a_shorter_sound_", inFileName)); 
		}
		...
		{
			printError(Messages.getString("Unsupported_file_type___Please_try_again_with_a_file_that_ends_in__wav___au___aif__or__aiff")); 
		}

		if(DEBUG)
		{
			System.out.println("New_sound_created_from_file__" + fileName); 
	...
	public void playAtRateDur(double rate, double durInFrames) throws SoundException
	{
		...
		{
			printError(Messages.getString("The_given_duration_in_frames__is_out_of_the_playable_range___Try_something_between_1_and_", durInFrames, getLengthInFrames())); 
		}
		if(rate > Float.MAX_VALUE)
		{
			printError(Messages.getString("The_new_sample_rate__is_out_of_the_playable_range___Try_something_between_0_and_", rate, Float.MAX_VALUE)); 
		}
	...
	{   
		...
		{
			printError(Messages.getString("You_are_trying_to_play_to_index_____The_sound_only_has_samples_total_", (endFrame+1), getAudioFileFormat().getFrameLength()) ); 
		}
		if(startFrame < 0)
		{
			printError(Messages.getString("You_cannot_start_playing_at_index____Choose_1_to_start_at_the_begining_", (startFrame+1))); 
		}
		if(endFrame < startFrame)
		{
			printError(Messages.getString("You_cannot_start_playing_at_index_and_stop_playing_at_index____The_start_index_must_be_before_the_stop_index_", (startFrame+1), (endFrame+1))); 
		}
	...
	public byte[] getFrame(int frameNum) throws SoundException
	{
		...
		{
			printError(Messages.getString("That_index___does_not_exist__The_last_valid_index_is_", (frameNum), (getAudioFileFormat().getFrameLength() -1) ));
		}
	...
	{
		System.out.println(Messages.getString("The_index_isn_t_valid_for_this_sound", index)); 
	}
	...
	public int getSampleValue(int frameNum) throws SoundException
	{
		...
		if(frameNum >= getAudioFileFormat().getFrameLength())
		{
			printError(Messages.getString("You_are_trying_to_access_the_sample_at_index____but_the_last_valid_index_is_at_", (frameNum), (getAudioFileFormat().getFrameLength() - 1)));
		}
		else if(frameNum < 0)
		{
		...
		if(format.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED))
		{
			...
			{
				printError(Messages.getString("Unsupported_audio_encoding___The_sample_size_is_not_recognized_as_a_standard_format_")); 
				return -1;
			}
		}
		...
		{
			printError(Messages.getString("Unsupported_audio_encoding_____Currently_only_PCM__ALAW_and_ULAW_are_supported___Please_try_again_with_a_different_file_", format.getEncoding() )); 
			return -1;
		}
	}//getSample(int)
	...
	public int getRightSample(int frameNum) throws SoundException
	{
		...
		{
			printError(Messages.getString("You_asked_for_the_sample_at_index_____This_number_is_less_than_zero___Please_try_again_using_an_index_in_the_range__0_", (frameNum+1), (getAudioFileFormat().getFrameLength() - 1) )); 
		}
		...
		{
			printError(Messages.getString("Only_stereo_sounds_have_different_right_and_left_samples___You_are_using_a_mono_sound__try_getSample___instead", frameNum));  
			return -1;
		}
		int sampleSizeInBits = format.getSampleSizeInBits();
		boolean isBigEndian = format.isBigEndian();
	...
	public void setFrame(int frameNum, byte[] theFrame) throws SoundException
	{
		if(frameNum >= getAudioFileFormat().getFrameLength())
		{
			printError(Messages.getString("That_frame__number___does_not_exist__The_last_valid_frame_number_is_", frameNum, (getAudioFileFormat().getFrameLength() - 1)));
		}
		int frameSize = getAudioFileFormat().getFormat().getFrameSize();
		if(frameSize != theFrame.length)
			printError(Messages.getString("Frame_size_doesn_t_match")); 
		for(int i = 0; i < frameSize; i++)
		{
	...
	public void setSampleValue(int frameNum, int sample) throws SoundException
	{
		...
		else if(format.getEncoding().equals(AudioFormat.Encoding.ALAW))
		{
			if((sample>Short.MAX_VALUE)||(sample<Short.MIN_VALUE))
				printError(Messages.getString("You_are_trying_to_set_the_sample_value_to__", sample, Short.MAX_VALUE, Short.MIN_VALUE)); 
			...
			throws SoundException
	{
		...
		if(format.getChannels() == 1)
			printError(Messages.getString("This_is_a_mono_sound___only_stereo_sounds_have_different_left_and_right_samples_")); 
	...
	public String toString()
	{
		...
		return Messages.getString("SimpleSound_file__length__", fileName, getLengthInBytes()); 
	}
