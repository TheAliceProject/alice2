...
public class BetterPlaceAnimation extends PointOfViewAnimation {
	...
	public class RuntimeBetterPlaceAnimation extends RuntimePointOfViewAnimation {
		...
		public void prologue( double t ) {
			...
			if( m_asSeenBy == null ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("needs_something_or_someone_to_move_to_", m_subject.name.getStringValue()), null, BetterPlaceAnimation.this.asSeenBy ); 
            }
            if( m_subject == m_asSeenBy ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("can_t_move_to_", m_subject.name.getStringValue(), m_subject.name.getStringValue()), getCurrentStack(), BetterPlaceAnimation.this.asSeenBy );              
            }
            
            if (m_subject.isAncestorOf(m_asSeenBy)) {
            	throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException(  Messages.getString("can_t_move_to_a_part_of_itself", m_subject.name.getStringValue()), getCurrentStack(), BetterPlaceAnimation.this.asSeenBy );             
            }
